...
for task in $tasks; do 
    for train_mode in $train_modes; do
        for version in $versions; do
            ...
            python off_moo_baselines/diffusion_guidance/experiment.py --version=$version --model=$model --task=$task --use_wandb=False --retrain_model=False --train_mode=$train_mode --seed=$seed
        done
    done 
done
