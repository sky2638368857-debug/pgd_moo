...
def get_preference_rankings_1(
    ...
    N = y.shape[0]
    contributions_all = {
        "score": np.zeros((N,)),   # diversity
        "pref": np.zeros((N,)),    # 新增：preference
    ...
    for e, front in enumerate(fronts):
        ...
        # -------- preference score --------
        # 假设是最小化问题 → 要取负号（越小越好 → 越大越优）
        pref_scores = -np.sum(y_front * w, axis=1)

        contributions_all["pref"][front] = pref_scores

        # -------- diversity score --------
        if metric == "hypervolume":
            contributions = get_ind_hypervolume_contribution(
                y_front, reference_point, task_name=task_name
            )
        elif metric == "crowding_distance":
            contributions = get_crowding_distance(y_front)
        ...
        # -------- 核心排序（字典序）--------
        # 先按 pref 排，再按 diversity 排（稳定排序）
        idx = np.arange(len(front))

        # 第三优先级：diversity（降序）
        idx = idx[np.argsort(contributions[idx])[::-1]]

        # 第二优先级：preference（降序，稳定覆盖）
        idx = idx[np.argsort(pref_scores[idx])[::-1]]

        # -------- 写回全局 rank --------
        for i in range(len(front)):
            contributions_all["rank"][front[idx[i]]] = curr_rank + i
...
def get_dataloader_1(X: np.ndarray,
                   y: np.ndarray,
                   X_test: np.ndarray,
                   y_test: np.ndarray,
                   X_pref: Optional[np.ndarray] = None,
...
class preference_dataset_ind_1(Dataset):
    def __init__(self, X, y, hv_contribution_rankings, diversity_score_threshold=0.0):
        self.X = X
        self.y = y
        self.hv_contribution_rankings = hv_contribution_rankings
    ...
    def __getitem__(self, idx):
        ...
        if self.hv_contribution_rankings["front"][idx] < self.hv_contribution_rankings["front"][idx_2]:
            ind = torch.tensor([0.0])
        elif self.hv_contribution_rankings["front"][idx] > self.hv_contribution_rankings["front"][idx_2]:
            ind = torch.tensor([1.0])
        else:
            ind = torch.tensor([2.0])
        return x_1, x_2, y_1, y_2, ind
